...
echo "
upstream django {
    server unix:///tmp/WebappServer.one.sock;
    server unix:///tmp/WebappServer.two.sock;
    server unix:///tmp/WebappServer.three.sock;
}
...
# Cron 定时更新证书(每月)
# crontab -e
# 0 0 1 * * /etc/nginx/certs/letsencrypt_cmd.sh $DOMAIN $DOMAIN_DIR "DNS:$DOMAIN,DNS:www.$DOMAIN,DNS:live.$DOMAIN,DNS:static.$DOMAIN" >> /var/log/lets-encrypt.log 2>&1
